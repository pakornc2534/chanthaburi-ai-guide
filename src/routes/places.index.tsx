  ...
  head: () => ({
    meta: [
      { title: "สถานที่ท่องเที่ยวจันทบุรี — Places in Chanthaburi" },
      {
        name: "description",
        content:
          "Browse all curated travel spots in Chanthaburi: beaches, waterfalls, temples and more.",
      },
    ],
  }),
});
  ...
  const filtered = useMemo(() => {
    return places.filter((p) => {
      ...
      if (q) {
        ...
        const hay =
          `${p.name_th} ${p.name_en} ${p.description_th} ${p.description_en} ${p.district ?? ""}`.toLowerCase();
